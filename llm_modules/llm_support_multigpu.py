...
def setup_distributed():
    """Initialize distributed training under SLURM or single-GPU mode."""
    if "SLURM_PROCID" in os.environ:
        rank = int(os.environ["SLURM_PROCID"])  # Global rank
        world_size = int(os.environ["SLURM_NTASKS"])  # Total number of tasks
        local_rank = int(os.environ["SLURM_LOCALID"])  # Local rank within the node
    elif torch.cuda.device_count() > 1:  # Local multi-GPU run
        rank, world_size, local_rank = 0, torch.cuda.device_count(), 0
        dist.init_process_group(backend="nccl", rank=rank, world_size=world_size)
    else:
        return 0, 1, 0  # Single-GPU fallback
    
    os.environ["MASTER_ADDR"] = os.getenv("SLURM_LAUNCH_NODE_IPADDR", "127.0.0.1")
    os.environ["MASTER_PORT"] = str(29500 + rank % 1000)  # Avoid conflicts
...
class LLM:
    ...
    def __init__(self, device: str = "cpu", quantization: str = None):
        ...
        print(f"LLM initialized on GPU {self.local_rank} (global rank {self.rank}) with quantization={self.quantization}")
    ...
    def load_model_and_tokenizer(self, hf_model_path: str, model_id: str, token: str, redownload: bool = False):
        ...
        print(f"Model and tokenizer loaded successfully on GPU {self.local_rank} (global rank {self.rank}).")
    ...
    def generate_response(self, conversation: List[Dict], max_new_tokens: int = 100, **kwargs) -> Optional[str]:
        ...
        # with torch.no_grad():
        #     gen_tokens = self.model.module.generate(tokenized_input, **gen_config)
